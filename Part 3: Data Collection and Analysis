This is where your plan comes to life. A well-organized process is key to getting high-quality data. For a practical field guide on online survey research here.
...
•	Normality Checks:
o	Visualize: Use histograms and Q-Q plots.
o	Coefficients: Check that skewness and kurtosis values are between -1 and +1.
o	Formal Tests: Use Shapiro-Wilk or Kolmogorov-Smirnov with caution, as they are sensitive to sample size.
...
•	Key Assumptions:
o	Linearity of Logit: The relationship between continuous predictors and the log-odds of the outcome is linear.
o	Independence: Observations are not clustered or repeated.
o	No Multicollinearity: Check that the VIF is less than 2.5.
o	Adequate Sample Size: Ensure at least 10-20 events per variable (EPV) to prevent overfitting.
•	Diagnostics:
o	Model Fit: Assess calibration (accuracy of predictions) and discrimination (ability to distinguish between outcomes).
o	Outliers: Investigate standardised Pearson residuals greater than 2.0.
 
3. Multiple Linear Regression: For Continuous Outcomes
This model is used for continuous outcome variables.

•	Key Assumptions:
o	Linearity, Normality & Homoscedasticity: Check these by plotting residuals versus predicted values. The plot should be random, with no clear pattern or fanning shape.
o	Independence: The Durbin-Watson statistic should be around 2.
o	No Multicollinearity: VIF should be less than 2.5.
o	Sufficient Sample Size: A common guideline is 20 subjects per variable.

•	Influential Outliers:
o	Check standardised residuals (values > 3 are extreme).
o	Assess Cook's Distance (values > 1 or > 4/n are influential).
